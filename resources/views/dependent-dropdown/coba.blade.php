...
<html lang="en">
    ...
    <script>
                ...
                let id = $(this).val();
                $('#sub_province').empty();
                $('#sub_province').append(`<option value="0" disabled selected>Processing...</option>`);
                $.ajax({
                type: 'GET',
                ...
                $('#sub_province').empty();
                $('#sub_province').append(`<option value="0" disabled selected>Select Sub Category*</option>`);
                response.forEach(element => {
                    $('#sub_province').append(`<option value="${element['id']}">${element['name']}</option>`);
                    });
                }
            });
        });
        ...
        {{-- <script>
            $(document).ready(function(){
                $('#kota').on('change', function(e){
                    ...
                    $.get('{{url('kota')}}/'+id, function(data){
                        console.log(id);
                        console.log(data);
                        $('#kota').empty();
                        $each(data, function(index, element){
                            $('#kota').append("<tr></td>"+element.province_id+"</td><td>"+element.id+"</td>"+"<td>"+element.name+"</td></tr>");
                        })
                    });
                });
            });
